<template>
    <div>
        ...
        <hr>
        <div class="questions" v-if="answers.length == 0">
            En Construcción
        </div>
...
export default {
    ...
    async mounted(){
        ...
        try {
            ...
            var val = await window.eel.getSPDAQuestions(this.wizard, 5)(val)
    ...
    methods: {
        ...
        async updateQuestions(wizard){
            ...
            this.$store.dispatch('setLoading')
            try {
                var val = await window.eel.getSPDAQuestions(wizard, 5)(val)
                this.questions = JSON.parse(val)
                for (let i = 0; i < this.questions.length; i++) {
                    setTimeout( async ()=>{ 
                        try{
