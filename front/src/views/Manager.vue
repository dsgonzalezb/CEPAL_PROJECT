<template>
    <div>
                ...
                <div class="cols-c" v-if="answers[i] != undefined">
                    ...
                    <div class="col-c" v-if="question['TIPO_1'] != 'NA'">
                        ...
                        <div v-if="question['TIPO_1']== 'selección unica'">
                            <v-select :options="question['RESPUESTAS']" v-model="answers[i]['dato_tex']"  label="TEXTO" />
                        </div>
        ...
        <div style="text-align: center" v-show="questions.length>0">
            <b-button variant="outline-warning" @click="''"><i class="far fa-save"></i> {{$t('spda.save')}}</b-button>
        </div>
    </div>
...
export default {
    ...
    async mounted(){
        ...
        try {
            ...
            var val = await window.eel.getSPDAQuestions(this.wizard, 3)(val)
    ...
    methods: {
        ...
        async updateQuestions(wizard){
            ...
            this.$store.dispatch('setLoading')
            try {
                var val = await window.eel.getSPDAQuestions(wizard, 3)(val)
                this.questions = JSON.parse(val)
                for (let i = 0; i < this.questions.length; i++) {
                    setTimeout( async ()=>{ 
                        try{
...
.wizard
    ...
    grid-template-columns: 50px 100px 50px 100px 50px 100px 50px 100px 50px
    div
        overflow: hidden
        display: grid
        align-items: center
