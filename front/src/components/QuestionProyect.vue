<template>
    <div>
        ...
        <div class="questions" v-if="answers.length>0 && isLoad" @mouseover="$forceUpdate()">
            <div class="question">
                {{$t('graphics.proyects.year')}}
                <div class="cols-c">
                    <div class="col-c">
                        <div class="inline">
                            <v-select v-model="year" :options="yearOpt2"></v-select>
                            <br>
                        </div>
                    </div>
                </div>
            ...
            <div class="question" v-for="(question, i) in questions" :key="i">
                ...
                <div v-show="question['VISIBLE'] == 1 && getCalculated(i)">
                    <b>{{question['ID_PREGUNTA']}} </b> <b v-if="question['TIPO_1'] == 'titulo'">{{question['PREGUNTA']}}</b> <span v-else>{{question['PREGUNTA']}}</span> <span class="help" v-show="question['AYUDA'] != 'NA'" v-b-tooltip.hover :title="question['AYUDA']" ><i class="fas fa-question-circle"></i></span>
...
export default {
    ...
    watch: {
        year : function(newValue){
            if(newValue != parseInt(localStorage.getItem('year'))+1) this.updateQuestions()
        },
    },
    async mounted(){
        for (let i = 0; i < 2035-localStorage.getItem('year'); i++) {
            this.yearOpt2.push(parseInt(localStorage.getItem('year'))+1+i)
            
        }
        this.actYear = localStorage.getItem('year')
        ...
        try{
            ...
            var val6 = await window.eel.getAnswerUserD( this.actYear, this.idDes)(val6)
    ...
    methods: {
        ...
        updateAns(){
            this.answer_shared = this.answers
            this.$emit("input", this.answer_shared)
        },
        getRefValue(ref){
